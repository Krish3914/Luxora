...
const Cart = () => {
      ...
      if (check === "null") {
        ...
        try {
          ...
          let response = await axios.post(
            "https://shopping-backend-neon.vercel.app/cart",
            {
  ...
  const increment = (itemId) => {
    setQuantity((prevoius) => ({
      ...prevoius,
      [itemId]: (prevoius[itemId] || 0) + 1,
    }));
  };
  const decrement = (itemId) => {
    setQuantity((prevoius) => ({
      ...prevoius,
      [itemId]: prevoius[itemId] >= 1 ? prevoius[itemId] - 1 : 0,
    }));
  };
  ...
  return (
    ...
    >
      ...
      {datas === "nothing" ? (
        <div
          style={{ display: "grid", placeItems: "center", alignSelf: "center" }}
        >
          <p>
            You will not a Login or signup.please login or signup To use a cart
            page
          </p>
          <a href="/login">
            <button className="btn-hover btn-cart">Login</button>
          </a>
          <p>OR</p>
          <a href="/signup">
            <button className="btn-hover btn-cart">Signup</button>
          </a>
        </div>
      ) : (
        <div className="cart_paging">
          {Object.values(datas).map((item, index) => (
            <div
              ...
              id={item._id}
              key={`div_tag${item.productName}`}
              data-aos="fade-up"
            >
              <img
                src={item.img[0].url}
                alt={`image_product${item.productName}`}
                id={`cart_img`}
              />
              <div className="cart_product_details">
                <div id="buttons_details">
                  ...
                  <button id="remove" onClick={() => remove(item._id)}>
                    <img
                      ...
                      alt={`image_delete_alts${index}`}
                      key={`image_delete_alts_index${index}`}
                    />
                  </button>
                </div>
                ...
                <div id="cart_offer">
                  ...
                  <span id="cart_offer_percentage">{` ${Math.floor(
                    ((item.MRPprize - item.SELLprize) / item.MRPprize) * 100
                  )}%  off`}</span>
                </div>
                ...
                <div id="buttons_details">
                  ...
                  <button
                    key={`incre${item.productName}`}
                    onClick={() => increment(index)}
                  >
                    +
                  </button>
                  <h4 id="current_prize_ctegory">TotalPrize:</h4>
                  <span id="current_prize">
                    ₹{item.SELLprize * quantity[index] || 0}
                  </span>
                </div>
                <div id="cart_options" style={{ gap: 13 }}>
                  <Link to={`/${item.category}/${item._id}`}>
                    <button id="view">
                      <img
                        src="https://cdn-icons-png.flaticon.com/128/535/535193.png"
                        alt={`image_alts${index}`}
                        key={`image_alts_index${index}`}
                      />
                    </button>
                  </Link>
                  <button
                  ...
                  >
                    <img
                      ...
                      alt={`image_order_alts${index}`}
                      key={`image_order_alts_index${index}`}
                    />
                    OrderNow
                  </button>
                </div>
